...
from .models import Activities, Visitors
...
def edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        # アクティビティのレコ―ド作成
        Activities.objects.create(user=request.user.account,content=visitor_object)
        visitor_object.save()
...
class  AccountRegistration(TemplateView):
    ...
    def post(self,request):
        ...
        if self.params["account_form"].is_valid() and self.params["add_account_form"].is_valid():
            ...
            message = "このメールは、送信専用です。ご返信いただいても対応できません。"
            from_email = 'toritoritorina@gmail.com'  # 送信者
            recipient_list = [account.email]  # 宛先リスト
